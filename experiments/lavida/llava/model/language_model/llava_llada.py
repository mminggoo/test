...
class LlavaLladaForMaskedDiffusion(LLaDAModelLM,LlavaMetaForCausalLM):
    ...
    def generate(
        ...
        **kwargs,
        ...
        visual_start_index = torch.where(inputs == -200)[0].tolist()[0]
        if images is not None:
            (inputs, position_ids, attention_mask, _, inputs_embeds, _, image_features) = self.prepare_inputs_labels_for_multimodal(inputs, position_ids, attention_mask, None, None, images, modalities, image_sizes=image_sizes)
        else:
            # breakpoint()
        ...
        visual_index = [visual_start_index, visual_start_index+image_features[0].shape[0]]
        #return super().generate(position_ids=position_ids, attention_mask=attention_mask, inputs_embeds=inputs_embeds, **kwargs)
        return llada_generate(self.get_model(),inputs_embeds=inputs_embeds,position_ids=position_ids,attention_mask=attention_mask,visual_index=visual_index,**kwargs)

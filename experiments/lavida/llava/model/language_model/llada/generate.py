...
def generate(model, prompt=None, steps=None, max_new_tokens=128, block_length=128, temperature=0.,
    ...
    if prefix_lm:
        if vfg_scale > 0.:
            ...
            past_key_values2 = model([None, None],input_embeddings=torch.cat([inputs_embeds, inputs_embeds_unv], dim=0),use_cache=True).attn_key_values
        else:
            past_key_values = model(None,input_embeddings=inputs_embeds,use_cache=True).attn_key_values
        # breakpoint()
    ...
    if 'vfg_start' in kwargs:
        vfg_start = int(kwargs['vfg_start']*steps)
        vfg_end = int(kwargs['vfg_end']*steps)
    ...
    for num_block in range(num_blocks):
        ...
        for i in range(steps):
            ...
            if cfg_scale > 0.:
                ...
                if vfg_scale > 0. and vfg_start <= i <= vfg_end:
                    inputs_embeds_curr = model.transformer.wte(x)
                    logits_v = model([None,None],input_embeddings=torch.cat([inputs_embeds_curr, inputs_embeds_curr], dim=0),past_key_values=past_key_values2).logits

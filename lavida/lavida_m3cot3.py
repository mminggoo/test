...
def run_eval(args, ccot=False):
    ...
    for idx, data in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        if generated_text.strip().upper()[-1] in ALPHA_MAP:
            a = generated_text.strip().upper()[-1]
            t = f'\\boxed{{{a}}}'
            tokens = tokenizer.tokenize(t)
            ids = tokenizer.convert_tokens_to_ids(tokens)
            indices = torch.nonzero(cont == ids[2], as_tuple=False)
            if len(indices) > 0:
                answer_steps.append(hist[indices[-1][1].item()])
                print(hist[indices[-1][1].item()])

    ans_file.close()
    print(f"[{total}] ACC: {acc:.4f}")
    with open("answer_steps_256_256.jsonl", "w") as f:

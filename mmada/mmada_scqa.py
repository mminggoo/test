...
NUM_MAP = [str(i) for i in range(100)]  # 0,1,2,... 최대 100지문까지 대응
import re
...
def run_eval(config, args, ccot=False):
    ...
    for data in tqdm(dataset):
        ...
        labels = NUM_MAP[:len(data['choices'])]
        ...
        # output_ids = model.mmu_generate(input_ids, max_new_tokens=2, steps=1, block_length=2)
        max_new_tokens = 2
        ...
        output_ids = model.mmu_generate(input_ids, max_new_tokens=max_new_tokens, steps=1, block_length=max_new_tokens, vfg_scale=1.5) # 0.5047
        end_time = time.time()

...
def run_eval(config, args, ccot=False):
    ...
    for idx, data in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        if ccot:
            ...
            sg_output_ids = model.mmu_generate(input_ids, max_new_tokens=128, steps=32, block_length=128)
        ...
        post_token = torch.tensor(uni_prompting.text_tokenizer(['Therefore, the correct answer is '])['input_ids']).to(device)
        post_idx = -9
        start_time = time.time()
        # 새로운 위치 인지 패널티 적용 generate 함수 사용
        max_new_tokens = 256
        output_ids = model.mmu_generate_posaware(
            input_ids,
        ...
        # output_ids = model.mmu_generate(input_ids, max_new_tokens=max_new_tokens, steps=1, block_length=max_new_tokens, vfg_scale=1.0)
#         output_ids = model.mmu_generate_with_last_remasking(input_ids, max_new_tokens=128, steps=64, block_length=128)
#         # output_ids, transfer_steps = model.mmu_generate_measure(input_ids, max_new_tokens=128, steps=64, block_length=128)
#         # print(f"Transfer steps: {transfer_steps}")
#         # print(output_ids)
        end_time = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pos-penalty-gamma", type=float, default=0.5)
    parser.add_argument("--pos-penalty-alpha", type=float, default=1.0)
